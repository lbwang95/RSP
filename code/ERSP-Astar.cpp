...
bool checkMBDom(int pid, double alpha){
    int lasteid = AllP[pid].lastid;
    for (int i = 0; i < Pe[lasteid].size();i++){
        int iterpid = Pe[lasteid][i];
        if(AllP[pid].mean>AllP[iterpid].mean){
            if(fetchTrav(AllP[pid].mean,AllP[pid].var,alpha)>fetchTrav(AllP[iterpid].mean,AllP[iterpid].var,alpha))
                return true;
        }
    }
    return false;
...
double ERSPQuery(int s, int t, double alpha){
    ...
    while(!SE.empty()){
        ...
        if (v == t)
            return optw=p.first;
        bool MBDflag = false;
        if(covflag[lasteid]==0){
            MBDflag = checkMBDom(pid, alpha);
        }
        for (int i = 0; i < adj[v].size();i++){
            ...
            if (covflag[lasteid] == 0 && MBDflag && cov[lasteid][thiseid] >= 0)
                continue;
...
int main(int argc , char * argv[]){
    ...
    for (int i = 0; i < M / 2 + 1;i++){
        ...
        double mincov = DBL_MAX, maxcov = -DBL_MAX;
        for (it = cov[i].begin(); it != cov[i].end();it++){
            mincov = min(mincov, it->second);
            maxcov = max(maxcov, it->second);
        }
        if(mincov>=0)
            covflag[i] = 1;
        else if(maxcov<0)
    ...
    freopen((prefix + sfile+sq+st+string("ERSPResults")).c_str(), "w", stdout);
